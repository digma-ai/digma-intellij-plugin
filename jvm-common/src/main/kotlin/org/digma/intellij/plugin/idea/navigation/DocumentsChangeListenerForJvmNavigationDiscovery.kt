 ...
 * installs a document listener when a document is opened in the editor and removed the listener when file closed.
 */
class DocumentsChangeListenerForJvmNavigationDiscovery(private val project: Project) : FileEditorManagerListener {
                    ...
                    override fun documentChanged(event: DocumentEvent) {
                        try {
                            ...
                            documentChangeAlarm.addRequest({
                                try {
                                    JvmSpanNavigationProvider.getInstance(project).documentChanged(event.document)
                                    JvmEndpointNavigationProvider.getInstance(project).documentChanged(event.document)
                                } catch (e: Throwable) {
