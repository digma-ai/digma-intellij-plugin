...
class CodeLensHost(project: Project) : LifetimedProjectComponent(project) {
    ...
    private fun installCodeLens(virtualFile: VirtualFile) {
        ...
        val job = componentLifetime.coroutineScope.launchWithErrorReporting("CodeLensHost.installCodeLens", logger) {
            if (logger.isTraceEnabled) {
                Log.log(logger::trace, "Installing code lens for {}", virtualFile)
            }
    ...
    private fun removeCodelens(virtualFile: VirtualFile) {
        ...
        val job = componentLifetime.coroutineScope.launchWithErrorReporting("CodeLensHost.removeCodelens", logger) {
            if (logger.isTraceEnabled) {
