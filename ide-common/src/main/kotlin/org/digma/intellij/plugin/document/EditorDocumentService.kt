...
class EditorDocumentService(private val project: Project, private val cs: CoroutineScope) : Disposable {
    ...
    // The worst thing that may happen is that we install document listeners on a file that is not in the project content. It does check isWritable
    // so we will not install document listener for non-writable files. A file not in content and is writable is very rare.
