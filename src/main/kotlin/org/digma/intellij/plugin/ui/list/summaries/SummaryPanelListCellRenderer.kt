...
class SummaryPanelListCellRenderer : AbstractPanelListCellRenderer() {
    ...
    override fun createPanel(project: Project,
        ...
        return when (val model = value.modelObject) {
            ...
            is SpanDurationChangeInsight.Change -> commonListItemPanel(buildSpanDuration(model, panelsLayoutHelper))
...
private fun buildSpanDuration(value: SpanDurationChangeInsight.Change, panelsLayoutHelper: PanelsLayoutHelper): JPanel {
    val title = JLabel(asHtml(value.span.displayName), SwingConstants.LEFT)
