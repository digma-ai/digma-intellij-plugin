...
import org.digma.intellij.plugin.idea.discovery.GLOBAL_OPENTELEMETY_FQN
import org.digma.intellij.plugin.idea.discovery.OPENTELEMETY_FQN
import org.digma.intellij.plugin.idea.discovery.TRACER_FQN
import org.digma.intellij.plugin.idea.discovery.WITH_SPAN_ANNOTATION_FQN
import org.digma.intellij.plugin.idea.discovery.WITH_SPAN_INST_LIBRARY_1
import org.digma.intellij.plugin.idea.discovery.WITH_SPAN_INST_LIBRARY_2
import org.digma.intellij.plugin.idea.discovery.WITH_SPAN_INST_LIBRARY_3
import org.digma.intellij.plugin.idea.discovery.WITH_SPAN_INST_LIBRARY_4
import org.digma.intellij.plugin.idea.discovery.WITH_SPAN_INST_LIBRARY_5
import org.digma.intellij.plugin.idea.discovery.createMethodCodeObjectId
import org.digma.intellij.plugin.idea.discovery.createSpanIdFromInstLibraryAndSpanName
import org.digma.intellij.plugin.idea.discovery.createSpanNameForWithSpanAnnotation
import org.digma.intellij.plugin.idea.discovery.getExpressionValue
import org.digma.intellij.plugin.idea.discovery.isMethodWithFirstArgumentString
...
internal fun getSpanInfoFromWithSpanAnnotatedMethod(uMethod: UMethod): List<SpanInfo> {
    ...
    if (withSpanAnnotation != null && containingFile != null && containingClass != null) {
        ...
        spanInfos.add(SpanInfo(createSpanIdFromInstLibraryAndSpanName(WITH_SPAN_INST_LIBRARY_1, spanName), spanName, methodId, containingFileUri,uMethod.startOffset))
        spanInfos.add(SpanInfo(createSpanIdFromInstLibraryAndSpanName(WITH_SPAN_INST_LIBRARY_2, spanName), spanName, methodId, containingFileUri,uMethod.startOffset))
        spanInfos.add(SpanInfo(createSpanIdFromInstLibraryAndSpanName(WITH_SPAN_INST_LIBRARY_3, spanName), spanName, methodId, containingFileUri,uMethod.startOffset))
        spanInfos.add(SpanInfo(createSpanIdFromInstLibraryAndSpanName(WITH_SPAN_INST_LIBRARY_4, spanName), spanName, methodId, containingFileUri,uMethod.startOffset))
        spanInfos.add(SpanInfo(createSpanIdFromInstLibraryAndSpanName(WITH_SPAN_INST_LIBRARY_5, spanName), spanName, methodId, containingFileUri,uMethod.startOffset))
        return spanInfos
    } else {
        return emptyList()
    }
...
fun getSpanInfoFromSpanBuilderMethodCallExpression(
    ...
    if (spanName != null && instLibrary != null) {
        ...
        return SpanInfo(spanId, spanName, methodId, containingFileUri,spanBuilderMethodCall.textRange?.startOffset ?: 0)
    }
