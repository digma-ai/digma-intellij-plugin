            ...
            return try {
                ...
                analyticsProvider?.let {
                    when (it.about.isCentralize) {
                        true -> AuthManagerCentralizedLoginHandlerBase(it)
                        false, null -> AuthManagerLocalLoginHandlerBase(it)
                    }
                } ?: AuthManagerNoOpLoginHandler("analytics provider is null")
                ...
                Log.log(logger::trace, "Exception in createLoginHandler , returning AuthManagerNoOpLoginHandler", analyticsProvider?.apiUrl)
                AuthManagerNoOpLoginHandler("error in getAbout $e")
            }
        }
    }
